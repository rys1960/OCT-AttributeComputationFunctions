...
void min_value(const double *input, int length, maxStruct * y);
				 ...
				 int nrhs, const mxArray *prhs[]) 
{
	...
	for (j=1;j<n;j++)
	{
		...
		//min_value((inputData+j*m),m,cm);
		//mexPrintf("min value is  %f with index %d\n", cm->min, cm->index);
		if (cm->max<EPS)
			conn = 3;
		else
			conn = connectivity;
		for (i=0;i<m;i++)
		{
			...
			//min_value((inputData+(j-1)*m+start),ending-start+1,cm);
			maxIndex = cm->index-1;
